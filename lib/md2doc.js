...
function textFromHtml(value) {
  return decodeHtml(
    String(value || "")
      ...
      .replace(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi, (anchor, attrs, body) => {
        const label = textFromHtml(body);
        const href = decodeHtml((attrs.match(/\bhref="([^"]*)"/i) || [])[1] || "");
        if (!/\bderivation-link\b/i.test(attrs)) {
          return href && href !== "#" ? `${label} (${href})` : label;
        }
        const slug = (attrs.match(/\bdata-derive-slug="([^"]*)"/i) || [])[1] || "";
        if (!slug) return label;
        return `${label} [derive:${decodeHtml(slug)}]`;
      })
      .replace(/<img\b[^>]*alt="([^"]*)"[^>]*>/gi, "［图片：$1］")
      .replace(/<br\s*\/?>/gi, "\n")
      .replace(/<\/p>\s*<p>/gi, "\n")
      .replace(/<[^>]+>/g, "")
  ).trim();
  ...
  while (index < source.length) {
    ...
    if (char === "\\") {
      ...
      if (command.name === "boxed") {
        const group = readBalanced(source, index, "{", "}");
        const scripted = applyMathScripts(
          `<m:borderBox><m:e>${mathOxml(group.value)}</m:e></m:borderBox>`,
          source,
          group.end
        );
        output += scripted.xml;
...
function richRunsFromHtml(html) {
  ...
  while (cursor < source.length) {
    const mathIndex = source
      .slice(cursor)
      .search(/<span class="markdown-math markdown-math-inline(?: [^"]*)?"[^>]*data-latex="/i);
...
function blocksFromRenderedHtmlV2(html) {
  ...
  while ((match = pattern.exec(html))) {
    if (match[1]) {
      output.push({ type: "heading", level: Number(match[1]), html: match[2], text: textFromHtml(match[2]) });
    } else if (match[3]) {
...
function documentXmlV2(markdown, title, options = {}, renderedResult = null) {
  const docxOptions = normalizeDocxOptions(options);
  const rendered = renderedResult || inspectMarkdown(markdown);
  const blocks = blocksFromRenderedHtmlV2(rendered.html);
  const body = [];
  if (title) body.push(paragraph(title, "Title", { docx: docxOptions }));
  for (const block of blocks) {
    if (block.type === "heading") body.push(paragraphFromHtml(block.html, headingStyleForLevel(block.level), { docx: docxOptions }));
...
function markdownToDocx({ markdown, title, options }) {
  const rendered = inspectMarkdown(markdown);
  if (!rendered.valid) {
    const error = new Error("Markdown 或 LaTeX 校验失败，已阻止 DOCX 导出。");
    error.code = "MD2FILE_VALIDATION_FAILED";
    error.diagnostics = rendered.diagnostics;
    throw error;
  }
  return zip([
    ...
    { name: "word/document.xml", content: documentXmlV2(markdown, title, options, rendered) },
    { name: "word/styles.xml", content: stylesXml() }
  ]);
}

module.exports = { inspectMarkdown, markdownToDocx, markdownRenderer };
