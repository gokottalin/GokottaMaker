...
function blocksFromRenderedHtmlV2(html) {
  const output = [];
  const pattern =
    /<h([2-4])[^>]*>([\s\S]*?)<\/h\1>|<div class="markdown-math markdown-math-display"[^>]*data-latex="([^"]*)"[^>]*>[\s\S]*?<\/div>|<p>([\s\S]*?)<\/p>|<pre[^>]*><code>([\s\S]*?)<\/code><\/pre>|<blockquote>([\s\S]*?)<\/blockquote>|<(ul|ol)>([\s\S]*?)<\/\7>|<div class="markdown-table-wrap">([\s\S]*?)<\/table><\/div>|<hr\s*\/>/gi;
  let match;
  while ((match = pattern.exec(html))) {
    if (match[1]) {
      output.push({ type: "heading", level: Number(match[1]), text: textFromHtml(match[2]) });
...
function documentXmlV2(markdown, title, options = {}) {
  ...
  for (const block of blocks) {
    if (block.type === "heading") body.push(paragraph(block.text, `Heading${Math.max(1, block.level - 1)}`, { docx: docxOptions }));
