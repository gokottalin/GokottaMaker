...
function createSeo({ siteUrl, allPosts, allProjects, allFormulas = () => [], text }) {
  ...
  function publicPages() {
    return [
      ...
      ...allFormulas().map((formula) => ({ loc: absoluteUrl(`/formula/${encodeURIComponent(formula.slug)}`), priority: "0.75", lastmod: formula.publishedAt }))
    ];
  }
