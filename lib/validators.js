...
function normalizedId(value, fallback) {
  ...
  if (!ID_PATTERN.test(id)) throw validationError("ID/slug 只能包含小写字母、数字和连字符，长度 2-97");
  return id;
}
...
function numberValue(value, field, { min = 0, max = 999999, fallback = 0 } = {}) {
  ...
  if (!Number.isFinite(number) || number < min || number > max) throw validationError(`${field}必须在 ${min}-${max} 之间`);
...
function coverPath(value, fallback) {
  ...
  if (!PATH_PATTERN.test(text) && !URL_PATTERN.test(text)) throw validationError("封面路径必须来自 assets/uploads 或 HTTP URL");
...
function optionalUrl(value, field) {
  const text = stringValue(value);
  if (!text) return "";
  if (!URL_PATTERN.test(text) && !PATH_PATTERN.test(text)) throw validationError(`${field}必须是 HTTP URL 或站内资源路径`);
...
function validatePostPayload(payload) {
  ...
  return {
    ...
    featuredOrder: numberValue(payload.featuredOrder, "精选排序", { min: 0, max: 999 }),
...
function validateUploadPayload(payload) {
  ...
  if (!dataUrl.startsWith("data:image/")) throw validationError("上传内容必须是图片 data URL");
