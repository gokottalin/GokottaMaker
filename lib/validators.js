...
function commonLevelValue(value) {
  if (value === undefined) return undefined;
  if (value === null || (typeof value === "string" && value.trim() === "")) return 1;
  if (typeof value === "boolean" || typeof value === "object") {
    throw validationError("常用等级必须是 1-10 的整数；主动清空按 1 处理");
  }
  const text = String(value).trim();
  if (!/^(?:10|[1-9])$/.test(text)) {
    throw validationError("常用等级必须是 1-10 的整数；主动清空按 1 处理");
...
function validateCommonLevelPayload(payload) {
  const contentType = enumValue(
    payload?.contentType,
    ["article", "project", "formula", "derivation", "focus", "miniapp"],
    "",
    "内容类型"
  );
  return {
    contentType,
    contentId: requiredText(payload?.contentId, "内容 ID", 160),
    commonLevel: commonLevelValue(payload?.commonLevel) ?? 5
  };
}

function validateHomepageFocusPayload(payload) {
  if (!Array.isArray(payload?.slots) || payload.slots.length !== 3) {
    throw validationError("聚焦内容必须一次提交 large、small-1、small-2 三个槽位");
  }
  const allowed = ["large", "small-1", "small-2"];
  const slots = payload.slots.map((item) => ({
    slot: enumValue(item?.slot, allowed, "", "聚焦槽位"),
    postId: requiredText(item?.postId, "聚焦文章 ID", 160),
    commonLevel: commonLevelValue(item?.commonLevel)
  }));
  if (new Set(slots.map((item) => item.slot)).size !== 3) {
    throw validationError("聚焦内容三个槽位必须完整且互异");
  }
  if (new Set(slots.map((item) => item.postId)).size !== 3) {
    throw validationError("聚焦内容必须选择三篇互异文章");
  }
  return { slots: allowed.map((slot) => slots.find((item) => item.slot === slot)) };
}
...
function formulaSelectionIdentity({ displayName, moduleKey, categoryPath, latex }) {
  const name = requiredText(normalizedMetadataText(displayName), "公式名称", 160);
  const module = normalizedFormulaModule(moduleKey);
  const category = normalizedFormulaCategoryPath(categoryPath);
  const formulaLatex = preservedText(latex, "公式 LaTeX", 20000, { required: true });
  const normalizedName = name.toLowerCase();
  const identityLatex = formulaLatex.replace(/\r\n?/g, "\n").trim();
  const digest = sourceTextHash(
    JSON.stringify([module, category.toLowerCase(), normalizedName, identityLatex])
  ).slice(0, 12);
  const readable = normalizedName
    ...
    .replace(/-+/g, "-")
    .replace(/^-|-$/g, "");
  const base = (readable || module || "formula").slice(0, 60).replace(/-$/g, "") || "formula";
...
function normalizedFormulaCategoryPath(value) {
  ...
  if (
    ...
    throw validationError("公式分类最多两级（大类/小类），请使用 1-2 个有效名称并以 / 分隔");
  }
  if (segments.some((segment) => /^(?:l[1-9]\d*|第?[一二三四五六七八九十\d]+层)$/iu.test(segment))) {
    throw validationError("公式分类不能使用 L1/L2/L3 等动态推导层级");
  }
  return categoryPath;
}
...
function validateFormulaBusinessPayload(payload) {
  ...
  return {
    ...
    commonLevel: commonLevelValue(payload.commonLevel)
  };
}

function validateFormulaCardPayload(payload) {
  return {
    ...validateFormulaBusinessPayload(payload),
...
function validateFormulaMetadataOperationPayload(payload) {
  const action = enumValue(
    payload?.action,
    ["preview", "rename", "merge", "migrate", "archive", "restore", "delete"],
    "",
    "元数据操作"
  );
  const kind = enumValue(payload?.kind, ["module", "category", "tag", "card"], "", "元数据对象类型");
  const ids = Array.isArray(payload?.formulaIds)
    ? [...new Set(payload.formulaIds.map(normalizedFormulaId))]
    : [];
  if (ids.length > 200) throw validationError("单次最多处理 200 张公式卡");
  const targetModuleKey = payload?.targetModuleKey ? normalizedFormulaModule(payload.targetModuleKey) : "";
  const targetCategoryPath = payload?.targetCategoryPath
    ? normalizedFormulaCategoryPath(payload.targetCategoryPath)
    : "";
  let displayName = payload?.displayName
    ? requiredText(normalizedMetadataText(payload.displayName), "新名称", 160)
    : "";
  if (displayName && kind === "category") displayName = normalizedFormulaCategoryPath(displayName);
  if (displayName && kind === "tag") displayName = normalizedFormulaTags([displayName])[0];
  if (["rename", "merge"].includes(action) && !displayName) throw validationError("重命名或合并必须提供新名称");
  if (action === "migrate" && (!targetModuleKey || !targetCategoryPath)) {
    throw validationError("迁移必须选择目标模块和分类");
  }
  return {
    action,
    kind,
    classificationId: optionalText(payload?.classificationId, "分类 ID", 160),
    formulaIds: ids,
    displayName,
    targetModuleKey,
    targetCategoryPath,
    backupConfirmed: booleanValue(payload?.backupConfirmed),
    confirmText: optionalText(payload?.confirmText, "二次确认文本", 80)
  };
}
...
function validateFormulaContentBindingPayload(payload) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    throw validationError("公式绑定请求格式不正确");
  }
  const sourceKind = enumValue(payload.sourceKind, ["article", "formula_revision"], "", "公式绑定来源类型");
  const sourceId =
    sourceKind === "article"
      ? normalizedId(payload.sourceId, "")
      : normalizedFormulaRevisionId(payload.sourceId);
  const sourceFormulaId =
    sourceKind === "formula_revision" ? normalizedFormulaId(payload.sourceFormulaId) : "";
  if (!Array.isArray(payload.bindings) || payload.bindings.length > 500) {
    throw validationError("公式绑定列表必须是数组且不能超过 500 项");
  }
  const targets = new Set();
  const bindings = payload.bindings.map((raw, index) => {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
      throw validationError(`第 ${index + 1} 项公式绑定格式不正确`);
    }
    const targetFormulaId = normalizedFormulaId(raw.targetFormulaId || raw.formulaId);
    if (targets.has(targetFormulaId)) {
      throw validationError(`同一来源不能重复绑定公式：${targetFormulaId}`);
    }
    targets.add(targetFormulaId);
    const locations = Array.isArray(raw.locations) ? raw.locations : [];
    if (locations.length > 1000) throw validationError("单个公式绑定的位置不能超过 1000 项");
    return {
      targetFormulaId,
      targetRevisionId: raw.targetRevisionId
        ? normalizedFormulaRevisionId(raw.targetRevisionId)
        : null,
      displayMode: enumValue(raw.displayMode, ["inline", "display"], "display", "公式显示模式"),
      ordinal: numberValue(raw.ordinal, "公式绑定顺序", { min: 0, max: 100000, fallback: index }),
      locations: locations.map((location, locationIndex) => {
        if (!location || typeof location !== "object" || Array.isArray(location)) {
          throw validationError(`第 ${index + 1} 项公式绑定位置格式不正确`);
        }
        const bindingId = stringValue(location.bindingId);
        if (bindingId && !FORMULA_BINDING_PATTERN.test(bindingId)) {
          throw validationError("公式绑定位置 bindingId 格式不正确");
        }
        return {
          bindingId,
          revisionId: location.revisionId
            ? normalizedFormulaRevisionId(location.revisionId)
            : null,
          displayMode: enumValue(
            location.displayMode,
            ["inline", "display"],
            raw.displayMode || "display",
            "公式绑定位置显示模式"
          ),
          ordinal: numberValue(location.ordinal, "公式绑定位置顺序", {
            min: 0,
            max: 100000,
  ...
  return { sourceKind, sourceId, sourceFormulaId, bindings };
}
