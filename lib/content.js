...
function formulaContentBindingId(sourceKind, sourceId, targetFormulaId) {
  const digest = crypto
    .createHash("sha256")
    .update(JSON.stringify([sourceKind, sourceId, targetFormulaId]))
    .digest("hex")
    .slice(0, 32);
  return `fbind.${digest}`;
}
...
function createContentStore(db) {
  ...
  function hasColumn(table, column) {
    if (!schemaColumns.has(table)) {
      schemaColumns.set(table, new Set(db.prepare(`PRAGMA table_info(${table})`).all().map((item) => item.name)));
    }
    return schemaColumns.get(table).has(column);
  }
  function optionalColumn(table, column, { prefix = "", fallback = "NULL", coalesce = "" } = {}) {
    if (!hasColumn(table, column)) return fallback;
    const reference = `${prefix}${column}`;
    return coalesce ? `COALESCE(${reference}, ${coalesce})` : reference;
  }
  ...
  function decorateFormulaContentBinding(row) {
    if (!row) return null;
    let location = {};
    try {
    ...
    const { locationJson, ...binding } = row;
    return { ...binding, location };
  }

  function formulaContentBindingsForSource(sourceKind, sourceId, options = {}) {
    const lifecycleClause = options.includeRetired ? "" : "AND binding.lifecycle_status = 'active'";
    return db
      .prepare(
        `SELECT binding.binding_id AS bindingId, binding.source_kind AS sourceKind,
                binding.source_id AS sourceId, binding.source_formula_id AS sourceFormulaId,
                binding.target_formula_id AS targetFormulaId,
                binding.target_revision_id AS targetRevisionId,
                binding.display_mode AS displayMode, binding.ordinal,
                binding.location_json AS locationJson,
                binding.lifecycle_status AS lifecycleStatus,
                binding.created_at AS createdAt, binding.updated_at AS updatedAt,
                target.slug AS targetSlug, target.display_name AS targetDisplayName,
                target.publish_status AS targetPublishStatus,
                target.archived_at AS targetArchivedAt
         FROM formula_content_bindings binding
         JOIN formula_cards target ON target.formula_id = binding.target_formula_id
         WHERE binding.source_kind = ? AND binding.source_id = ? ${lifecycleClause}
         ORDER BY binding.ordinal, binding.binding_id`
      )
      .all(sourceKind, sourceId)
      .map(decorateFormulaContentBinding);
  }

  function formulaContentBindingsForTarget(targetFormulaId, options = {}) {
    const lifecycleClause = options.includeRetired ? "" : "AND binding.lifecycle_status = 'active'";
    return db
      .prepare(
        `SELECT binding.binding_id AS bindingId, binding.source_kind AS sourceKind,
                ...
                binding.created_at AS createdAt, binding.updated_at AS updatedAt
         FROM formula_content_bindings binding
         WHERE binding.target_formula_id = ? ${lifecycleClause}
         ORDER BY binding.source_kind, binding.source_id, binding.ordinal, binding.binding_id`
      )
      .all(targetFormulaId)
      .map(decorateFormulaContentBinding);
  }

  function formulaArticleReferrers(targetFormulaId, options = {}) {
    const admin = options.admin === true;
    const visibility = admin
      ? ""
      : "AND post.deleted_at IS NULL AND post.publish_status = 'published'";
    return db
      .prepare(
        `SELECT binding.binding_id AS bindingId, post.id AS postId, post.slug,
                post.title, post.publish_status AS publishStatus,
                post.deleted_at AS deletedAt, binding.location_json AS locationJson,
                binding.ordinal, binding.updated_at AS updatedAt
         FROM formula_content_bindings binding
         JOIN posts post ON post.id = binding.source_id
         WHERE binding.source_kind = 'article'
           AND binding.target_formula_id = ?
           AND binding.lifecycle_status = 'active'
           ${visibility}
         ORDER BY post.title, post.id, binding.ordinal, binding.binding_id`
      )
      .all(targetFormulaId)
      .map((row) => {
        let references = [];
        try {
          const location = JSON.parse(row.locationJson || "{}");
          references = Array.isArray(location.references) ? location.references : [];
        } catch {
          references = [];
        }
        const base = {
          ...
          postId: row.postId,
          slug: row.slug || row.postId,
          title: row.title,
          route: `./post.html?id=${encodeURIComponent(row.postId)}`,
          referenceCount: Math.max(1, references.length),
          references,
        ...
        return {
          ...
          lifecycleState: row.deletedAt ? "deleted" : row.publishStatus
        };
      });
  }

  function saveFormulaContentBindings(payload) {
    return withTransaction(() => {
      if (payload.sourceKind === "article") {
        if (!db.prepare("SELECT 1 AS present FROM posts WHERE id = ?").get(payload.sourceId)) {
          throw contentError(404, "公式绑定来源文章不存在");
        }
      } else if (
        !db
          .prepare("SELECT 1 AS present FROM formula_revisions WHERE revision_id = ? AND formula_id = ?")
          .get(payload.sourceId, payload.sourceFormulaId)
      ) {
        throw contentError(400, "公式绑定来源修订不属于来源公式");
      }

      const desiredTargets = new Set(payload.bindings.map((binding) => binding.targetFormulaId));
      const retire = db.prepare(`
        UPDATE formula_content_bindings
        SET lifecycle_status = 'retired', updated_at = CURRENT_TIMESTAMP
        WHERE source_kind = ? AND source_id = ? AND target_formula_id = ?
      `);
      for (const existing of formulaContentBindingsForSource(payload.sourceKind, payload.sourceId, {
        includeRetired: true
      })) {
        if (!desiredTargets.has(existing.targetFormulaId) && existing.lifecycleStatus === "active") {
          retire.run(payload.sourceKind, payload.sourceId, existing.targetFormulaId);
        }
      }
      ...
      const upsert = db.prepare(`
        INSERT INTO formula_content_bindings
          (binding_id, source_kind, source_id, source_formula_id, target_formula_id,
           target_revision_id, display_mode, ordinal, location_json, lifecycle_status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')
        ON CONFLICT(source_kind, source_id, target_formula_id) DO UPDATE SET
      ...
      try {
        for (const binding of payload.bindings) {
          const target = db
            .prepare("SELECT archived_at AS archivedAt FROM formula_cards WHERE formula_id = ?")
            .get(binding.targetFormulaId);
          const existingBinding = db
            ...
            .get(payload.sourceKind, payload.sourceId, binding.targetFormulaId);
          if (target?.archivedAt && existingBinding) target.archivedAt = null;
          if (!target) throw contentError(400, `公式绑定目标不存在：${binding.targetFormulaId}`);
          if (target.archivedAt) throw contentError(409, `已归档公式不能建立新绑定：${binding.targetFormulaId}`);
          upsert.run(
            formulaContentBindingId(payload.sourceKind, payload.sourceId, binding.targetFormulaId),
            payload.sourceKind,
            payload.sourceId,
            ...
            binding.ordinal,
            JSON.stringify({
              references: binding.locations,
              provenance: binding.provenance || "authority"
            })
          );
        }
      } catch (error) {
        if (/formula binding (?:self-reference|cycle)/i.test(error.message)) {
          throw contentError(409, error.message);
        }
        throw error;
      }
      return formulaContentBindingsForSource(payload.sourceKind, payload.sourceId);
    });
  }
    ...
    return db
      .prepare(
        `SELECT id, slug, 'post' AS type, title, category, category_key AS categoryKey,
                ...
                ${optionalColumn("posts", "common_level", { fallback: "5", coalesce: "5" })} AS commonLevel,
                ${optionalColumn("posts", "view_count", { fallback: "0" })} AS viewCount,
    ...
    return db
      .prepare(
        `SELECT id, slug, 'project' AS type, title, status, status_key AS statusKey,
                ...
                ${optionalColumn("projects", "reading_minutes")} AS readingMinutes,
                ${optionalColumn("projects", "common_level", { fallback: "5", coalesce: "5" })} AS commonLevel,
                ${optionalColumn("projects", "view_count", { fallback: "0" })} AS viewCount,
  ...
  function knowledgeNodeColumns() {
    return `id, slug, 'knowledge_node' AS type, node_type AS nodeType, symbol, title,
            ...
            ${optionalColumn("knowledge_nodes", "common_level", { fallback: "5", coalesce: "5" })} AS commonLevel,
            ${optionalColumn("knowledge_nodes", "view_count", { fallback: "0" })} AS viewCount,
  ...
  function saveKnowledgeNode(payload) {
    return withTransaction(() => {
      ...
      if (payload.commonLevel !== undefined && hasColumn("knowledge_nodes", "common_level")) {
        db.prepare("UPDATE knowledge_nodes SET common_level = ? WHERE id = ?").run(payload.commonLevel, payload.id);
      }
  ...
  function formulaCardColumns(options = {}) {
    ...
    return `c.formula_id AS formulaId, c.slug, ${displayName} AS displayName,
            ...
            ${optionalColumn("formula_cards", "common_level", { prefix: "c.", fallback: "5", coalesce: "5" })} AS commonLevel,
            ${optionalColumn("formula_cards", "view_count", { prefix: "c.", fallback: "0" })} AS viewCount,
  ...
  function publicFormulaCardBySlug(slug, options = {}) {
    const key = String(slug || "");
    if (!key) return null;
    const row = db
      .prepare(
    ...
    return {
      ...
      graph: formulaGraphForCard(card.formulaId, {
        admin: false,
        revisionId: card.publishedRevisionId,
        payloadNodeLimit: options.payloadNodeLimit
      })
    };
  }
  ...
  function resolveLegacyFormulaRedirect(slug) {
    ...
    return {
      ...
      statusCode: 308,
      location: `/formula/${encodeURIComponent(row.targetSlug)}`
    };
  }
  ...
  function formulaDerivationForCard(id, options = {}) {
    ...
    const dependencies = db
      .prepare(
        `SELECT target.formula_id AS formulaId, target.slug,
                ...
                revision.latex,
                COALESCE(json_extract(binding.location_json, '$.provenance'), 'authority') AS provenance,
                binding.ordinal
         FROM formula_content_bindings binding
         JOIN formula_cards target ON target.formula_id = binding.target_formula_id
         LEFT JOIN formula_revisions revision ON revision.revision_id = ${targetPointer}
         WHERE binding.source_kind = 'formula_revision'
           AND binding.source_id = ?
           AND binding.lifecycle_status = 'active'
           ${publicTargetFilter}
         ORDER BY binding.ordinal ASC, target.formula_id ASC`
      )
      .all(revisionId)
    ...
    const incoming = db
      .prepare(
        `SELECT source.formula_id AS formulaId, source.slug,
                ...
                binding.ordinal
         FROM formula_content_bindings binding
         JOIN formula_cards source ON source.formula_id = binding.source_formula_id
         LEFT JOIN formula_revisions revision ON revision.revision_id = ${sourcePointer}
         WHERE binding.source_kind = 'formula_revision'
           AND binding.target_formula_id = ?
           AND binding.source_id = ${sourcePointer}
           AND binding.lifecycle_status = 'active'
           ${publicSourceFilter}
    ...
    const result = {
      ...
      incoming,
      dependencies,
      articleReferrers: formulaArticleReferrers(formulaId, { admin }),
      ...
      result.affectedSources = db
        .prepare(
          `WITH RECURSIVE affected(formula_id) AS (
             SELECT ?
             UNION
             SELECT binding.source_formula_id
             FROM formula_content_bindings binding
             JOIN formula_cards source ON source.formula_id = binding.source_formula_id
             JOIN affected ON binding.target_formula_id = affected.formula_id
             WHERE binding.source_kind = 'formula_revision'
               AND binding.source_id = source.current_revision_id
               AND binding.lifecycle_status = 'active'
           )
  ...
  function formulaGraphForCard(id, options = {}) {
    const formulaId = String(id || "");
    const admin = options.admin === true;
    const payloadNodeLimit = Math.max(
      FORMULA_GRAPH_PAYLOAD_NODE_LIMIT,
      Math.min(Number(options.payloadNodeLimit || FORMULA_GRAPH_PAYLOAD_NODE_LIMIT), 5000)
    );
    ...
    const allEdges = db
      .prepare(
        `SELECT binding.source_formula_id AS sourceFormulaId,
                binding.target_formula_id AS targetFormulaId,
                binding.ordinal,
                COALESCE(json_extract(binding.location_json, '$.provenance'), 'authority') AS provenance
         FROM formula_content_bindings binding
         ...
         WHERE binding.source_kind = 'formula_revision'
           ...
           AND binding.lifecycle_status = 'active'
           ${edgeVisibility}
         ORDER BY binding.source_formula_id ASC,
                  binding.ordinal ASC,
                  binding.target_formula_id ASC`
      )
      .all()
      .filter(
    ...
    while (queue.length && componentIds.length < payloadNodeLimit) {
    ...
    const articleReferrerMap = new Map();
    const articleEdges = [];
    for (const targetFormulaId of componentIds) {
      const targetNodeId = keyFor(targetFormulaId);
      for (const article of formulaArticleReferrers(targetFormulaId, { admin })) {
        const articleNodeId = `article:${article.postId}`;
        const prior = articleReferrerMap.get(articleNodeId);
        const targetRank = Number(layers.get(targetFormulaId) || 0) - rootLayer;
        if (!prior) {
          const base = {
            id: articleNodeId,
            nodeType: "article",
            ...
            direction: "article",
            current: false,
            initiallyVisible: initialSet.has(targetFormulaId)
          };
          articleReferrerMap.set(
            articleNodeId,
            admin
              ? {
                  ...base,
                  ...
                  lifecycleState: article.lifecycleState
                }
              : base
          );
        } else {
          prior.referenceCount += article.referenceCount;
          prior.rank = Math.min(Number(prior.rank || 0), targetRank - 1);
          prior.initiallyVisible ||= initialSet.has(targetFormulaId);
        }
        articleEdges.push({
          id: `article-edge:${article.postId}:${targetNodeId}`,
          edgeType: "article_reference",
    ...
    const articleNodes = [...articleReferrerMap.values()];
    const initialArticleIds = articleNodes
      .filter((node) => node.initiallyVisible)
      .map((node) => node.id);
    return {
      mode: admin ? "current" : "published",
      currentNodeId: keyFor(formulaId),
      nodes: [...articleNodes, ...nodes],
      edges: [...articleEdges, ...edges],
      initialNodeIds: [...initialArticleIds, ...initialNodeIds],
      expandableNodeIds,
      hiddenNodeCount: Math.max(0, componentIds.length - initialNodeIds.length),
      truncated: reachableCount > componentIds.length,
      limits: {
        initialNodes: FORMULA_GRAPH_INITIAL_NODE_LIMIT,
        payloadNodes: payloadNodeLimit
      }
    };
  }
  ...
  function validateFormulaDependencyTargets(formulaId, targetFormulaIds, options = {}) {
    if (options.requirePublishedDependencies === true) {
      const targetLookup = db.prepare(
      ...
      for (const targetFormulaId of targetFormulaIds) {
        const target = targetLookup.get(targetFormulaId);
        if (!target) throw contentError(400, `公式依赖目标不存在：${targetFormulaId}`);
        if (target.publishStatus === "archived" || target.archivedAt) {
          throw contentError(409, `已归档公式不能作为依赖：${targetFormulaId}`);
        }
        if (target.publishStatus !== "published" || !target.publishedRevisionId) {
          throw contentError(409, `未发布公式不能作为依赖：${targetFormulaId}`);
        }
        if (target.currentRevisionId !== target.publishedRevisionId) {
          throw contentError(409, `存在待发布修订的公式不能作为新依赖：${targetFormulaId}`);
        }
      }
    }
  ...
  function listFormulaDependencyCandidates(filters = {}) {
    const sourceFormulaId = String(filters.sourceFormulaId || "").trim();
    ...
    const publishedWhere = [
      ...
      "c.current_revision_id IS NOT NULL",
      "c.current_revision_id = c.published_revision_id",
      `EXISTS (
        SELECT 1 FROM formula_revision_publications publication
        WHERE publication.formula_id = c.formula_id
          AND publication.revision_id = c.published_revision_id
      )`
    ];
    ...
    if (query) {
      ...
      where.push(
        `(c.display_name LIKE ? ESCAPE '\\' OR c.formula_id LIKE ? ESCAPE '\\' OR
          c.module_key LIKE ? ESCAPE '\\' OR c.category_path LIKE ? ESCAPE '\\')`
      );
    ...
    const rows = db
      .prepare(
        `SELECT c.formula_id AS formulaId, c.display_name AS displayName,
                ...
                revision.latex
         FROM formula_cards c
         JOIN formula_revisions revision ON revision.revision_id = c.published_revision_id
         WHERE ${whereSql}
         ORDER BY c.module_key ASC, c.category_path ASC, c.display_name ASC, c.formula_id ASC
         LIMIT ? OFFSET ?`
      )
      .all(...params, pageSize, (effectivePage - 1) * pageSize);
    const baseFacetWhere = publishedWhere.join(" AND ");
    const modules = db
      .prepare(
        `SELECT c.module_key AS moduleKey
         FROM formula_cards c
         WHERE ${baseFacetWhere}
         GROUP BY c.module_key
         ORDER BY c.module_key ASC`
      )
      .all()
      .map((row) => row.moduleKey);
    const categories = db
      .prepare(
        `SELECT c.module_key AS moduleKey, c.category_path AS categoryPath
         FROM formula_cards c
         WHERE ${baseFacetWhere}
         GROUP BY c.module_key, c.category_path
         ORDER BY c.module_key ASC, c.category_path ASC`
      )
      .all();
    const items = rows.map((row) => {
      let disabledReason = "";
      if (sourceFormulaId && row.formulaId === sourceFormulaId) {
        disabledReason = "当前公式不能依赖自身";
      } else if (sourceFormulaId) {
        try {
          validateFormulaDependencyGraph({
            formulaIds: db
              .prepare("SELECT formula_id AS formulaId FROM formula_cards")
              .all()
              .map((item) => item.formulaId),
            edges: [
              ...currentFormulaDependencyEdges([sourceFormulaId]),
              { sourceFormulaId, targetFormulaId: row.formulaId, ordinal: 0 }
            ]
          });
        } catch (error) {
          if (Number(error?.status || 0) === 409 || /循环|cycle/i.test(String(error?.message || ""))) {
            disabledReason = "选择后会形成间接循环依赖";
          } else {
            throw error;
          }
        }
      }
      return { ...row, disabled: Boolean(disabledReason), disabledReason };
    });
    return {
      items,
      facets: { modules, categories },
      selection: { moduleKey, categoryPath, query },
  ...
  function syncFormulaDependencyAuthority(
    revisionId,
    formulaId,
    targetFormulaIds,
    provenance = "markdown"
  ) {
    return saveFormulaContentBindings({
      sourceKind: "formula_revision",
      sourceId: revisionId,
      sourceFormulaId: formulaId,
      bindings: targetFormulaIds.map((targetFormulaId, ordinal) => ({
        targetFormulaId,
  ...
  function formulaCardReferenceImpact(formulaId) {
    const card = formulaCardByIdAny(formulaId);
    if (!card) throw notFound("formula card not found");
    const articleRows = db.prepare(
      `SELECT b.post_id AS postId, p.publish_status AS publishStatus, b.revision_id AS revisionId
       FROM article_formula_bindings b JOIN posts p ON p.id = b.post_id
       WHERE b.formula_id = ? ORDER BY b.post_id`
    ).all(formulaId);
    const dependencyRows = db.prepare(
      `SELECT source_formula_id AS sourceFormulaId, revision_id AS revisionId
       FROM formula_revision_dependencies WHERE target_formula_id = ?
       ORDER BY source_formula_id, revision_id`
    ).all(formulaId);
    const historyRows = db.prepare(
      `SELECT source_kind AS sourceKind, source_id AS sourceId, lifecycle_status AS lifecycleStatus
       FROM formula_content_bindings WHERE target_formula_id = ?
       ORDER BY source_kind, source_id`
    ).all(formulaId);
    const reasons = [];
    if (articleRows.length) reasons.push(`${articleRows.length} 个文章引用`);
    if (dependencyRows.length) reasons.push(`${dependencyRows.length} 个公式依赖`);
    if (historyRows.length) reasons.push(`${historyRows.length} 个当前或历史绑定`);
    return {
      formulaId,
      publishStatus: card.publishStatus,
      articleCount: articleRows.length,
      draftArticleCount: articleRows.filter((row) => row.publishStatus === "draft").length,
      publishedArticleCount: articleRows.filter((row) => row.publishStatus === "published").length,
      dependencyCount: dependencyRows.length,
      historyReferenceCount: historyRows.length,
      articleReferences: articleRows,
      formulaDependencies: dependencyRows,
      historyReferences: historyRows,
      protected: reasons.length > 0,
      reasons
    };
  }

  function formulaMetadataImpact(payload) {
    let formulaIds = payload.formulaIds || [];
    let classification = null;
    if (payload.classificationId) {
      classification = listFormulaClassifications().find((item) => item.classificationId === payload.classificationId);
      if (!classification) throw notFound("formula classification not found");
      if (classification.kind === "module") {
        formulaIds = db.prepare("SELECT formula_id AS formulaId FROM formula_cards WHERE module_key = ?")
          .all(classification.slug).map((row) => row.formulaId);
      } else if (classification.kind === "category") {
        formulaIds = db.prepare("SELECT formula_id AS formulaId FROM formula_cards WHERE module_key = ? AND category_path = ?")
          .all(classification.parentSlug, classification.displayName).map((row) => row.formulaId);
      } else if (classification.kind === "tag") {
        formulaIds = db.prepare("SELECT formula_id AS formulaId FROM formula_card_tags WHERE tag_key = ?")
          .all(classification.displayName).map((row) => row.formulaId);
      }
    }
    formulaIds = [...new Set(formulaIds)];
    const cards = formulaIds.map(formulaCardReferenceImpact);
    return {
      classification,
      formulaIds,
      cardCount: cards.length,
      draftCount: cards.filter((card) => card.publishStatus === "draft").length,
      publishedCount: cards.filter((card) => card.publishStatus === "published").length,
      archivedCount: cards.filter((card) => card.publishStatus === "archived").length,
      articleCount: cards.reduce((sum, card) => sum + card.articleCount, 0),
      dependencyCount: cards.reduce((sum, card) => sum + card.dependencyCount, 0),
      protectedCount: cards.filter((card) => card.protected).length,
      cards
    };
  }

  function operateFormulaMetadata(payload) {
    return withTransaction(() => {
      const before = formulaMetadataImpact(payload);
      if (payload.action === "preview") return { action: "preview", impact: before };
      if (payload.action === "migrate") {
        assertFormulaClassificationsRegistered({
          moduleKey: payload.targetModuleKey,
          categoryPath: payload.targetCategoryPath,
          tags: []
        });
        const update = db.prepare(
          "UPDATE formula_cards SET module_key = ?, category_path = ?, updated_at = CURRENT_TIMESTAMP WHERE formula_id = ?"
        );
        for (const formulaId of before.formulaIds) update.run(payload.targetModuleKey, payload.targetCategoryPath, formulaId);
        return { action: "migrate", affectedCount: before.cardCount, impact: before };
      }
      if (payload.kind === "card") {
        if (payload.action === "archive" || payload.action === "restore") {
          const cards = before.formulaIds.map((id) =>
            payload.action === "archive" ? archiveFormulaCard(id, payload.actor) : restoreFormulaCard(id)
          );
          return { action: payload.action, affectedCount: cards.length, cards, impact: before };
        }
        if (payload.action === "delete") {
          if (!payload.backupConfirmed || payload.confirmText !== "PERMANENTLY DELETE") {
            throw contentError(409, "永久删除前必须确认已完成备份并输入 PERMANENTLY DELETE");
          }
          if (before.protectedCount) {
            const details = before.cards.filter((card) => card.protected)
              .map((card) => `${card.formulaId}：${card.reasons.join("、")}`).join("；");
            throw contentError(409, `永久删除已阻止：${details}`);
          }
          const triggerRows = db.prepare(
            "SELECT name, sql FROM sqlite_master WHERE type = 'trigger' AND tbl_name = 'formula_revisions' AND sql IS NOT NULL"
          ).all();
          for (const trigger of triggerRows) db.exec(`DROP TRIGGER ${trigger.name}`);
          const removeId = db.prepare("DELETE FROM formula_cards WHERE formula_id = ?");
          for (const formulaId of before.formulaIds) {
            db.prepare("DELETE FROM formula_revision_dependencies WHERE source_formula_id = ?").run(formulaId);
            db.prepare("DELETE FROM formula_content_bindings WHERE source_formula_id = ?").run(formulaId);
            db.prepare("DELETE FROM formula_reference_decisions WHERE formula_id = ? OR resolved_formula_id = ?").run(formulaId, formulaId);
            db.prepare("DELETE FROM formula_revision_publications WHERE formula_id = ?").run(formulaId);
            db.prepare("DELETE FROM formula_revisions WHERE formula_id = ?").run(formulaId);
            removeId.run(formulaId);
          }
          for (const trigger of triggerRows) db.exec(trigger.sql);
          return { action: "delete", affectedCount: before.cardCount, impact: before };
        }
      }
      const source = before.classification;
      if (!source) throw contentError(400, "分类操作必须提供 classificationId");
      if (["rename", "merge"].includes(payload.action)) {
        const normalizedName = normalizedClassificationName(payload.displayName);
        const target = listFormulaClassifications({ kind: source.kind, parentSlug: source.parentSlug })
          .find((item) => item.normalizedName === normalizedName && item.classificationId !== source.classificationId);
        if (payload.action === "merge" && !target) throw contentError(409, "合并目标不存在，请先选择已有同类选项");
        if (payload.action === "rename" && target) throw contentError(409, "新名称已存在；请改用合并操作");
        const nextName = target?.displayName || payload.displayName;
        if (source.kind === "module") {
          const nextSlug = target?.slug || generatedClassificationSlug(nextName);
          if (target) {
            const sourceCategories = listFormulaClassifications({ kind: "category", parentSlug: source.slug });
            const targetCategories = listFormulaClassifications({ kind: "category", parentSlug: nextSlug });
            for (const sourceCategory of sourceCategories) {
              const same = targetCategories.find((item) => item.normalizedName === sourceCategory.normalizedName);
              if (same) {
                db.prepare("UPDATE formula_cards SET category_path = ? WHERE module_key = ? AND category_path = ?")
                  .run(same.displayName, source.slug, sourceCategory.displayName);
                db.prepare("DELETE FROM formula_classifications WHERE classification_id = ?").run(sourceCategory.classificationId);
              } else {
                db.prepare("UPDATE formula_classifications SET parent_slug = ? WHERE classification_id = ?")
                  .run(nextSlug, sourceCategory.classificationId);
              }
            }
          }
          db.prepare("UPDATE formula_cards SET module_key = ? WHERE module_key = ?").run(nextSlug, source.slug);
          if (!target) db.prepare("UPDATE formula_classifications SET parent_slug = ? WHERE kind = 'category' AND parent_slug = ?").run(nextSlug, source.slug);
        } else if (source.kind === "category") {
          db.prepare("UPDATE formula_cards SET category_path = ? WHERE module_key = ? AND category_path = ?")
            .run(nextName, source.parentSlug, source.displayName);
        } else {
          const separator = nextName.indexOf(":");
          db.prepare(
            `INSERT OR IGNORE INTO formula_card_tags (formula_id, tag_key, namespace, value)
             SELECT formula_id, ?, ?, ? FROM formula_card_tags WHERE tag_key = ?`
          ).run(nextName, nextName.slice(0, separator), nextName.slice(separator + 1), source.displayName);
          db.prepare("DELETE FROM formula_card_tags WHERE tag_key = ?").run(source.displayName);
        }
        if (target) db.prepare("DELETE FROM formula_classifications WHERE classification_id = ?").run(source.classificationId);
        else db.prepare(
          "UPDATE formula_classifications SET slug = ?, display_name = ?, normalized_name = ?, updated_at = CURRENT_TIMESTAMP WHERE classification_id = ?"
        ).run(generatedClassificationSlug(nextName), nextName, normalizedName, source.classificationId);
        return { action: payload.action, affectedCount: before.cardCount, impact: before };
      }
      if (payload.action === "delete") {
        if (before.cardCount) throw contentError(409, "分类仍有公式卡引用；请先迁移或归档相关卡片");
        if (!payload.backupConfirmed || payload.confirmText !== "PERMANENTLY DELETE") {
          throw contentError(409, "删除分类前必须确认已完成备份并输入 PERMANENTLY DELETE");
        }
        db.prepare("DELETE FROM formula_classifications WHERE classification_id = ?").run(source.classificationId);
        return { action: "delete", affectedCount: 1, impact: before };
      }
      throw contentError(400, `不支持的元数据操作：${payload.action}`);
    });
  }
  ...
  function syncPostFormulaBindings(postId, markdown, options = {}) {
    ...
    const referencesByFormula = new Map();
    for (const reference of references) {
      if (!referencesByFormula.has(reference.formulaId)) {
        referencesByFormula.set(reference.formulaId, []);
      }
      referencesByFormula.get(reference.formulaId).push(reference);
    }
    saveFormulaContentBindings({
      sourceKind: "article",
      sourceId: postId,
      sourceFormulaId: "",
      bindings: [...referencesByFormula.entries()].map(([targetFormulaId, locations]) => {
        const ordered = [...locations].sort((left, right) => left.ordinal - right.ordinal);
        const revisionIds = [...new Set(ordered.map((location) => location.revisionId))];
        return {
          targetFormulaId,
          targetRevisionId: revisionIds.length === 1 ? revisionIds[0] : null,
          displayMode: ordered[0].displayMode,
  ...
  function insertFormulaRevision(payload, actor = {}) {
    ...
    validateFormulaDependencyTargets(payload.formulaId, dependencyFormulaIds, {
      requirePublishedDependencies: payload.requirePublishedDependencies === true
    });
    ...
    if (existing) {
      ...
      syncFormulaDependencyAuthority(revisionId, payload.formulaId, dependencyFormulaIds);
  ...
  function saveFormulaCard(payload) {
    return withTransaction(() => {
      ...
      validateFormulaDependencyTargets(
        ...
        { requirePublishedDependencies: effectivePayload.requirePublishedDependencies === true }
      );
      ...
      if (effectivePayload.commonLevel !== undefined && hasColumn("formula_cards", "common_level")) {
        db.prepare("UPDATE formula_cards SET common_level = ? WHERE formula_id = ?").run(
          effectivePayload.commonLevel,
          effectivePayload.formulaId
        );
      }
  ...
  function formulaIdentityCandidate(baseIdentity, sequence) {
    const suffix = sequence > 1 ? `-${sequence}` : "";
    const stem = baseIdentity.slug
      .slice(0, 80 - suffix.length)
      .replace(/-+$/g, "") || "formula";
    const slug = `${stem}${suffix}`;
    return { formulaId: `formula.user.${slug}`, slug };
  }

  function allocateFormulaIdentity(payload) {
    const baseIdentity = formulaSelectionIdentity(payload);
    for (let sequence = 1; sequence <= 10000; sequence += 1) {
      const candidate = formulaIdentityCandidate(baseIdentity, sequence);
      if (!formulaCardByIdAny(candidate.formulaId) && !formulaCardBySlugAny(candidate.slug)) {
        return candidate;
      }
    }
    throw contentError(409, "无法为公式分配唯一技术标识，请调整公式名称或分类后重试");
  }

  function createFormulaCard(payload) {
    return withTransaction(() => {
      const identity = allocateFormulaIdentity(payload);
      return saveFormulaCard({ ...payload, ...identity });
    });
  }

  function updateFormulaCard(id, payload) {
    return withTransaction(() => {
      const existing = formulaCardByIdAny(id);
      if (!existing) throw notFound("formula card not found");
      return saveFormulaCard({
        ...payload,
    ...
    return withTransaction(() => {
      ...
      for (const card of cards) {
        for (const revision of card.revisions) {
          ...
          syncFormulaDependencyAuthority(
            revision.revisionId,
            card.formulaId,
            revision.dependencyFormulaIds || []
          );
        }
      }
  ...
  function savePost(payload) {
    return withTransaction(() => {
      ...
      if (payload.commonLevel !== undefined && hasColumn("posts", "common_level")) {
        db.prepare("UPDATE posts SET common_level = ? WHERE id = ?").run(payload.commonLevel, payload.id);
      }
  ...
  function createFormulaFromSelection(
    ...
    return withTransaction(() => {
      ...
      const savedFormula = createFormulaCard({ ...formula, actor });
  ...
  function saveProject(payload) {
    return withTransaction(() => {
      ...
      if (payload.commonLevel !== undefined && hasColumn("projects", "common_level")) {
        db.prepare("UPDATE projects SET common_level = ? WHERE id = ?").run(payload.commonLevel, payload.id);
      }
  ...
  function hardDeletePost(id, options = {}) {
    return withTransaction(() => {
      ...
      db.prepare(
        `UPDATE formula_content_bindings
         SET lifecycle_status = 'retired', updated_at = CURRENT_TIMESTAMP
         WHERE source_kind = 'article' AND source_id = ? AND lifecycle_status = 'active'`
      ).run(id);
  ...
  function discoveryIdentityColumn(contentType) {
    if (contentType === "article") return { table: "posts", id: "id", aliases: ["id", "slug"] };
    if (contentType === "project") return { table: "projects", id: "id", aliases: ["id", "slug"] };
    if (contentType === "derivation") return { table: "knowledge_nodes", id: "id", aliases: ["id", "slug"] };
    if (contentType === "formula") return { table: "formula_cards", id: "formula_id", aliases: ["formula_id", "slug"] };
    return null;
  }

  function setCommonLevel(contentType, contentId, commonLevel) {
    return withTransaction(() => {
      if (contentType === "miniapp") {
        db.prepare(
          `INSERT INTO discovery_aux_content (content_type, content_id, common_level, view_count, updated_at)
           VALUES ('miniapp', ?, ?, 0, CURRENT_TIMESTAMP)
           ON CONFLICT(content_type, content_id) DO UPDATE SET
             common_level = excluded.common_level, updated_at = CURRENT_TIMESTAMP`
        ).run(contentId, commonLevel);
        return { contentType, contentId, commonLevel };
      }
      if (contentType === "focus") {
        const changed = db.prepare(
          "UPDATE homepage_focus_slots SET common_level = ?, updated_at = CURRENT_TIMESTAMP WHERE slot = ?"
        ).run(commonLevel, contentId);
        if (!changed.changes) throw notFound("focus slot not found");
        return { contentType, contentId, commonLevel };
      }
      const identity = discoveryIdentityColumn(contentType);
      if (!identity) throw contentError(400, "unsupported discovery content type");
      const where = identity.aliases.map((column) => `${column} = ?`).join(" OR ");
      const row = db.prepare(`SELECT ${identity.id} AS contentId FROM ${identity.table} WHERE ${where} LIMIT 1`)
        .get(...identity.aliases.map(() => contentId));
      if (!row) throw notFound("discovery content not found");
      db.prepare(`UPDATE ${identity.table} SET common_level = ? WHERE ${identity.id} = ?`).run(commonLevel, row.contentId);
      return { contentType, contentId: row.contentId, commonLevel };
    });
  }

  function incrementPublicView(contentType, contentId) {
    return withTransaction(() => {
      if (contentType === "miniapp") {
        db.prepare(
          `INSERT INTO discovery_aux_content (content_type, content_id, common_level, view_count, updated_at)
           VALUES ('miniapp', ?, 5, 1, CURRENT_TIMESTAMP)
           ON CONFLICT(content_type, content_id) DO UPDATE SET
             view_count = discovery_aux_content.view_count + 1`
        ).run(contentId);
        return Number(db.prepare(
          "SELECT view_count AS viewCount FROM discovery_aux_content WHERE content_type = 'miniapp' AND content_id = ?"
        ).get(contentId).viewCount);
      }
      const specifications = {
        article: { table: "posts", id: "id", where: "publish_status = 'published' AND deleted_at IS NULL" },
        project: { table: "projects", id: "id", where: "visibility_status = 'published' AND deleted_at IS NULL" },
        derivation: { table: "knowledge_nodes", id: "id", where: "publish_status = 'published' AND visibility_status IN ('public', 'unlisted') AND deleted_at IS NULL" },
        formula: { table: "formula_cards", id: "formula_id", where: "publish_status = 'published' AND archived_at IS NULL AND published_revision_id IS NOT NULL" }
      };
      const specification = specifications[contentType];
      if (!specification) throw contentError(400, "unsupported view content type");
      const result = db.prepare(
        `UPDATE ${specification.table}
         SET view_count = view_count + 1
         WHERE ${specification.id} = ? AND ${specification.where}`
      ).run(contentId);
      if (!result.changes) throw notFound("public content not found");
      return Number(db.prepare(
        `SELECT view_count AS viewCount FROM ${specification.table} WHERE ${specification.id} = ?`
      ).get(contentId).viewCount);
    });
  }

  function publicDiscoveryItems(contentType) {
    if (contentType === "article") {
      return db.prepare(
        `SELECT id, slug, title AS name, category, category_key AS module,
                tags AS keywords, cover, reading_minutes AS durationMinutes,
                COALESCE(common_level, 5) AS commonLevel, view_count AS viewCount,
                COALESCE(published_at, updated_at, date) AS publicAt,
                './post.html?id=' || id AS route
         FROM posts
    ...
    if (contentType === "project") {
      return db.prepare(
        `SELECT id, slug, title AS name, status AS category, status_key AS module,
                tags AS keywords, cover, reading_minutes AS durationMinutes,
                COALESCE(common_level, 5) AS commonLevel, view_count AS viewCount,
                COALESCE(published_at, updated_at, date) AS publicAt,
                './project.html?id=' || id AS route
         FROM projects
         WHERE visibility_status = 'published' AND deleted_at IS NULL`
      ).all();
    }
    if (contentType === "derivation") {
      return db.prepare(
        `SELECT id, slug, title AS name, node_type AS category, '' AS module, symbol,
                tags AS keywords, cover, NULL AS durationMinutes,
                COALESCE(common_level, 5) AS commonLevel, view_count AS viewCount,
                COALESCE(published_at, updated_at) AS publicAt,
                './derive.html?node=' || slug AS route
         FROM knowledge_nodes
    ...
    if (contentType === "formula") {
      return db.prepare(
        `SELECT c.formula_id AS id, c.slug,
                COALESCE(NULLIF(r.display_name, ''), c.display_name) AS name,
                COALESCE(NULLIF(r.category_path, ''), c.category_path) AS category,
                COALESCE(NULLIF(r.module_key, ''), c.module_key) AS module,
                COALESCE(r.tags_json, '[]') AS keywords,
                '' AS cover, NULL AS durationMinutes,
                COALESCE(c.common_level, 5) AS commonLevel, c.view_count AS viewCount,
                publication.published_at AS publicAt,
                '/formula/' || c.slug AS route,
                r.latex AS latex
         FROM formula_cards c
         JOIN formula_revisions r ON r.revision_id = c.published_revision_id
         JOIN formula_revision_publications publication
           ON publication.revision_id = c.published_revision_id
          AND publication.formula_id = c.formula_id
         WHERE c.publish_status = 'published' AND c.archived_at IS NULL`
      ).all();
    }
    throw contentError(400, "unsupported public discovery content type");
  }

  function homepageFocusSlots(admin = false) {
    const rows = db.prepare(
      `SELECT focus.slot, focus.post_id AS postId,
              COALESCE(focus.common_level, 5) AS commonLevel,
              focus.assigned_at AS assignedAt, focus.updated_at AS updatedAt,
              post.title, post.slug, post.cover, post.category,
              post.publish_status AS publishStatus, post.deleted_at AS deletedAt
       FROM homepage_focus_slots focus
       LEFT JOIN posts post ON post.id = focus.post_id
       ORDER BY CASE focus.slot WHEN 'large' THEN 0 WHEN 'small-1' THEN 1 ELSE 2 END`
    ).all();
    if (!admin) {
      return rows
        .filter((row) => row.publishStatus === "published" && !row.deletedAt)
        .map(({ commonLevel, publishStatus, deletedAt, ...row }) => ({ ...row, route: `./post.html?id=${encodeURIComponent(row.postId)}` }));
    }
    const items = rows.map((row) => ({
      ...row,
      available: row.publishStatus === "published" && !row.deletedAt,
      reasonCode: !row.title
        ? "FOCUS_POST_MISSING"
        : row.deletedAt
          ? "FOCUS_POST_ARCHIVED"
          : row.publishStatus !== "published"
            ? "FOCUS_POST_NOT_PUBLISHED"
            : "FOCUS_SLOT_AVAILABLE"
    }));
    return {
      slots: items,
      configured: items.length === 3,
      complete: items.length === 3 && items.every((item) => item.available),
      missingSlots: ["large", "small-1", "small-2"].filter((slot) => !items.some((item) => item.slot === slot && item.available))
    };
  }

  function saveHomepageFocusSlots(payload, actor = {}) {
    return withTransaction(() => {
      for (const item of payload.slots) {
        const post = db.prepare(
          "SELECT id, title FROM posts WHERE id = ? AND publish_status = 'published' AND deleted_at IS NULL"
        ).get(item.postId);
        if (!post) {
          const error = contentError(409, `聚焦槽位 ${item.slot} 只能选择已发布文章`);
          error.reasonCode = "FOCUS_POST_NOT_PUBLIC";
          throw error;
        }
      }
      const actorInfo = actorFields(actor);
      db.prepare("DELETE FROM homepage_focus_slots").run();
      const insert = db.prepare(
        `INSERT INTO homepage_focus_slots
          (slot, post_id, common_level, assigned_by_user_id, assigned_by_username)
         VALUES (?, ?, ?, ?, ?)`
      );
      for (const item of payload.slots) {
        insert.run(item.slot, item.postId, item.commonLevel === undefined ? 5 : item.commonLevel, actorInfo.id, actorInfo.username);
      }
      return homepageFocusSlots(true);
    });
  }

  function miniappDiscoveryMetadata(contentId) {
    return db.prepare(
      `SELECT content_id AS contentId, COALESCE(common_level, 5) AS commonLevel,
              view_count AS viewCount, updated_at AS updatedAt
       FROM discovery_aux_content WHERE content_type = 'miniapp' AND content_id = ?`
    ).get(contentId) || { contentId, commonLevel: 5, viewCount: 0, updatedAt: "" };
  }

  function latestPublicFormulas(limit = 8) {
    return publicDiscoveryItems("formula")
      .sort((left, right) => String(right.publicAt || "").localeCompare(String(left.publicAt || "")) || left.id.localeCompare(right.id))
      .slice(0, Math.max(0, Math.min(Number(limit || 8), 8)))
      .map(({ commonLevel, ...item }) => ({ ...item, formulaId: item.id, formulaRef: `{{formula-ref:${item.id}}}` }));
  }

  function allFormulaCardsForDiscovery() {
    return db.prepare(
      `SELECT ${formulaCardColumns()}
       FROM formula_cards c
       LEFT JOIN formula_revisions r ON r.revision_id = c.current_revision_id
       ORDER BY c.formula_id ASC`
    ).all().map(decorateFormulaCard);
  }

  function publicFormulaUploadSources() {
    return db.prepare(
      `SELECT revision.markdown_derivation AS markdown
       FROM formula_cards card
       JOIN formula_revisions revision ON revision.revision_id = card.published_revision_id
       JOIN formula_revision_publications publication
         ON publication.revision_id = card.published_revision_id
        AND publication.formula_id = card.formula_id
       WHERE card.publish_status = 'published' AND card.archived_at IS NULL`
    ).all().map((row) => row.markdown || "");
  }
  ...
  return {
    ...
    formulaMetadataImpact,
    operateFormulaMetadata,
    formulaBindingsForPost,
    formulaContentBindingsForSource,
    formulaContentBindingsForTarget,
    saveFormulaContentBindings,
