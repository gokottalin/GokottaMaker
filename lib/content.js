...
const { extractFormulaDependencyReferences } = require("./validators");
...
function createContentStore(db) {
  ...
  function formulaDerivationNode(row, options = {}) {
    ...
    if (!options.admin) {
      return {
        ...
        available: row.publishStatus === "published",
        latex: row.latex || "",
  ...
  function formulaDerivationForCard(id, options = {}) {
    ...
    const publicTargetFilter = admin ? "" : "AND target.publish_status = 'published'";
    const publicSourceFilter = admin ? "" : "AND source.publish_status = 'published'";
      ...
      result.publicationBlockers = db
        .prepare(
          `SELECT target.formula_id AS formulaId, target.display_name AS displayName,
           ...
           JOIN formula_cards target ON target.formula_id = dependency.target_formula_id
           WHERE dependency.revision_id = ?
             AND (target.publish_status <> 'published' OR target.published_revision_id IS NULL)
  ...
  function formulaGraphForCard(id, options = {}) {
    ...
    const visibility = admin
      ...
      : "card.publish_status = 'published' AND card.published_revision_id IS NOT NULL";
  ...
  function validateFormulaDependencyTargets(formulaId, targetFormulaIds) {
    const seen = new Set();
    for (const targetFormulaId of targetFormulaIds) {
      if (seen.has(targetFormulaId)) {
        throw contentError(400, `Markdown 推导存在重复公式依赖：${targetFormulaId}`);
      }
      if (targetFormulaId === formulaId) throw contentError(400, "公式卡不能依赖自身");
      if (!formulaCardByIdAny(targetFormulaId)) {
        throw contentError(400, `公式依赖目标不存在：${targetFormulaId}`);
      }
