...
约束 R2 必须 terminal_connected(A,B)。`
  ...
  async function buildCircuit() {
    ...
    try {
      ...
      image.alt = "LarkixElec 生成的 SVG 原理图预览";
      ...
      setIr(null);
      setLog(formatError(error, "接口未接入或请求失败。请确认 LarkixMaker 后端已实现 POST /api/elec/build。"), "error");
    } finally {
  ...
  async function copyLlmHandoff(mode) {
    ...
    try {
      ...
      setLog(formatError(error, `LLM 对接接口未接入。请确认 LarkixMaker 后端已实现 GET /api/elec/llm-handoff?mode=${mode}。`), "error");
    } finally {
  ...
  el.downloadSvgButton.addEventListener("click", () => {
    if (currentSvg) downloadText("larkixelec.svg", currentSvg, "image/svg+xml;charset=utf-8");
  });
