...
当前仓库版本为 `V2.5.5`（`package.json` 为 `2.5.5`，站点构建标识为 `V2.5.5+20260911-0001`）。

## 当前版本更新

`V2.5.5` 在一体化内容工作台基础上完善公式推导工作流：

- 新增独立公式详情页与兼容旧链接的规范跳转，公开内容严格受发布状态保护。
- 公式推导图采用唯一 JSON 契约，支持分支、合并、深层推导与安全分页续载。
- CMS 公式元数据支持两级分类、重命名、合并、迁移、归档、恢复及受保护删除。
- 高公式在访客端和 CMS 中自适应高度，保持完整展示且不产生内部纵向滚动。
