...
公式库把每条 L1 设计公式保存为一张唯一公式卡。公式卡使用稳定 `formulaId` 与 ASCII 路由 `slug`；二者建立后不可修改。每条不可变修订同时保存结论 LaTeX 与独立 Markdown 推导正文；任一正文发生变化都会形成新修订，旧修订只读保留。仅修改名称、模块、主分类、用途或标签不会制造重复修订。

公式卡状态固定为草稿、已发布和已归档。新卡默认为草稿，只在 CMS 可见；作者明确发布后，访客单卡才读取 `publishedRevisionId`。已发布卡继续编辑 LaTeX 或 Markdown 时，系统只更新作者当前修订并显示“待发布”，访客仍读取上一发布修订，直到作者再次明确发布。归档会隐藏独立访客页，但保留稳定身份、发布历史、不可变修订、文章绑定和推导关系；恢复后原路由继续有效。

管理端按“模块 / 主分类”加载，未选择分类时不返回整库列表。模块和主分类为可搜索单选，标签为可搜索多选；不存在的选项必须由作者明确新建。服务端只规范化元数据中的 Unicode、空格和安全标点并生成 slug，不改写 LaTeX 或 Markdown。选定分类后可搜索名称、用途、标识、LaTeX 或推导正文，并按标签、三态状态和分页浏览。访客单卡地址为：
...
需要复用公式库时，我在 Markdown 编辑区右键，或使用页面边缘固定的 `∑` 键盘入口。抽屉可按模块、自定义分类、`namespace:value` 标签和关键词搜索，并以行内或块级方式插入。插入后正文保存稳定的公式卡身份与不可变修订身份；后续修改公式卡不会静默改变文章引用的修订。

需要把正文公式建立为新公式卡时，必须先精确选中一个完整的行内或块级 LaTeX 表达式。选区不能混入正文、缺少定界符或同时包含多个公式。建卡时必须人工填写公式名称、模块和主分类；用途、标签与 Markdown 推导可选，系统不从上下文猜测这些字段。新卡默认为草稿，因此该原子建卡入口只用于草稿文章；公式卡发布后，文章才能继续发布。失败时原 LaTeX 与本地草稿保持不变。

长文编辑时，我可在正文顶部、中部或底部使用固定的 `∑` 入口展开公式抽屉。抽屉开合、搜索、快速预览及独立公式工作台往返均保留 Markdown、正文滚动、页面滚动、光标和最后一次有效公式选区；插入仍落在打开抽屉前捕获的位置。快速预览只显示该卡实际可插入的修订：草稿卡显示当前 CMS 修订，已发布卡显示已发布修订，待发布修改不会混入已发布预览。完整 Markdown 推导、发布状态、分类和依赖图继续在独立公式工作台维护。

草稿公式卡可以绑定草稿文章。文章切换为已发布时，服务端逐项检查绑定：草稿公式卡、待发布修订和从未发布的归档修订都会以明确原因阻止文章发布。已归档卡不能建立新绑定；既有文章继续按原 `revisionId` 渲染曾发布的历史修订。
...
公式卡只修改名称、模块、主分类、用途或标签时，文章不产生版本待决事项。公式卡 LaTeX 或 Markdown 生成新修订，或公式卡被归档时，每一篇仍绑定该公式卡的文章会各自产生一项黄色 CMS 待决事项；待决状态、处理人和处理时间均不进入访客 API 或公开正文。

待决期间，CMS 预览与访客文章继续按正文中保存的不可变 `revisionId` 渲染旧公式，不会自动跟随公式卡的当前修订。作者必须逐篇选择：

1. **保留旧修订**：正文和绑定均保持不变，只关闭该篇文章的待决事项。
2. **采用当前修订**：保留原 `bindingId`，把该篇文章的公式卡与 `revisionId` 显式更新到目标修订。已发布文章只能采用已经发布过的目标修订。
3. **另建公式卡**：以旧修订 LaTeX 和 Markdown 为初值，人工填写新名称、模块和主分类，可选填写用途与标签；系统生成新的技术标识，并保留原 `bindingId` 重新绑定。新卡仍为草稿，因此当前文章必须先保存为草稿。
...
推导网络的每个节点必须是独立公式卡。作者在当前修订的 Markdown 推导正文中插入 `{{formula-ref:<formulaId>}}`，一张公式卡可以引用多个直接依赖，多个上游也可以汇入同一依赖。正文中的短码是依赖边的唯一编辑来源；网络图只读取修订依赖，不保存第二份可编辑关系。

每次保存先在单个事务内解析全部短码，再校验目标存在、无自引用、无重复依赖、无悬空引用且整个有向图无直接或多层循环。校验失败时不创建修订，也不改变既有依赖。删除或替换短码会创建新的不可变修订；旧修订及其依赖原样保留。旧线性设置、替换和移除 API 继续兼容，但新 CMS 以 Markdown 多依赖工作台为主。

CMS 图谱固定读取 `currentRevisionId`，可显示草稿、待发布和归档边界。访客正文链接与网络图固定从 `publishedRevisionId` 遍历，只展示仍有公开修订的节点；草稿、待发布内容、归档目标名称以及内部修订信息不得进入公开载荷。大图首屏有节点上限，当前路径优先显示，旁支由读者按需展开。
